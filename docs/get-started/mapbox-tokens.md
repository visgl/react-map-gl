...
react-map-gl is open source and free. The underlying Mapbox GL JS library was free to use up to version 1.x, only requiring an access token if you load the map styles and tiles from Mapbox's data service. As of [v2.0](https://github.com/mapbox/mapbox-gl-js/releases/tag/v2.0.0), Mapbox GL JS must be used with an access token in order to access the map renderer.
...
It is possible to use the map component without the Mapbox service, if you use another tile source (for example, if you host your own map tiles). Note that this is no longer allowed using mapbox-gl v2. The options are:

- Stay on `react-map-gl@^5.0.0` and `mapbox-gl@^1.0.0`. react-map-gl plans to continue supporting this release in the foreseeable future, however, this version will not include any of the latest features of the map renderer, nor get any future updates from Mapbox.
